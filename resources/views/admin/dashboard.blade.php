...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        ...
        <h3 class="text-2xl font-black">{{ number_format($totalTickets, 0, ',', '.') }}</h3>
    </div>
    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-8 py-6">
                        ...
                        @elseif(strtolower($transaction->status) == 'expired')
                            <span class="px-3 py-1 bg-rose-100 text-rose-700 rounded-lg text-xs font-bold uppercase">Expired</span>
                        @else
                            <span class="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold uppercase">{{ $transaction->status }}</span>
                        @endif
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-8 py-12 text-center text-slate-400 font-semibold">Belum ada transaksi.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
